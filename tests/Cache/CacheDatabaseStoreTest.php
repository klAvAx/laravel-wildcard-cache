...
class CacheDatabaseStoreTest extends TestCase
{
    ...
    {
        $store = $this
            ->getMockBuilder(DatabaseStore::class)
            ->onlyMethods(['forgetIfExpired'])
        ...
        $getQuery
            ->shouldReceive('get')
            ->once()
            ->andReturn(collect([(object) ['key' => 'prefixfoo', 'expiration' => 1]]));
        ...
        $deleteQuery
            ...
            ->andReturn($deleteQuery);
        $deleteQuery
            ->shouldReceive('where')
            ->once()
            ->with('expiration', '<=', m::any())
            ->andReturn($deleteQuery);
        $deleteQuery
    ...
    public function testValueIsReturnedOnPostgres()
    {
        ...
        $table
            ...
            ->andReturn(collect([(object) [
                'key' => 'prefixfoo',
                'value' => base64_encode(serialize('bar')),
                'expiration' => 999999999999999
            ]]));
    ...
    public function testValueIsReturnedOnSqlite()
    {
        ...
        $table
            ...
            ->andReturn(collect([(object) [
                'key' => 'prefixfoo',
                'value' => base64_encode(serialize("\0bar\0")),
                'expiration' => 999999999999999
            ]]));
    ...
    public function testValueIsUpsertedOnPostgres()
    {
        $store = $this
            ->getMockBuilder(DatabaseStore::class)
            ->onlyMethods(['getTime'])
            ->setConstructorArgs($this->getPostgresMocks())
            ->getMock();
        $table = m::mock(stdClass::class);
        $store
            ->getConnection()
        ...
        $table
            ...
            ->with([[
                'key' => 'prefixfoo',
                'value' => base64_encode(serialize("\0")),
                'expiration' => 61
            ]], 'key')
            ->andReturn(1);
    ...
    public function testValueIsUpsertedOnSqlite()
    {
        $store = $this
            ->getMockBuilder(DatabaseStore::class)
            ->onlyMethods(['getTime'])
            ->setConstructorArgs($this->getSqliteMocks())
            ->getMock();
        $table = m::mock(stdClass::class);
        $store
            ->getConnection()
    ...
    public function testItemsMayBeRemovedFromCache()
    {
        ...
        $table
            ->shouldReceive('orWhereLike')
            ->once()
            ->with('key', 'prefixilluminate:cache:flexible:created:foo')
            ->andReturn($table);
        $table
            ->shouldReceive('delete')
            ->once();
    ...
    public function testSingleDynamicSegmentItemsMayBeRemovedFromCache()
    {
        ...
        $table
            ->shouldReceive('orWhereLike')
            ->once()
            ->with('key', 'prefixilluminate:cache:flexible:created:foo_%')
            ->andReturn($table);
        $table
            ->shouldReceive('delete')
            ->once();
    ...
    public function testMultipleDynamicSegmentItemsMayBeRemovedFromCache()
    {
        ...
        $table
            ->shouldReceive('orWhereLike')
            ->once()
            ->with('key', 'prefixfoo_%_bar_%')
            ->andReturn($table);
        $table
            ->shouldReceive('orWhereLike')
            ->once()
            ->with('key', 'prefixilluminate:cache:flexible:created:foo_%_bar_%')
            ->andReturn($table);
        $table
            ->shouldReceive('delete')
            ->once();
    ...
    public function testIncrementReturnsCorrectValues()
    {
        ...
        $table
            ...
            ->andReturn(null);
        $this
            ->assertFalse($store->increment('foo'));
        ...
        $table
            ...
            ->once()
            ->with(['value' => serialize(3)]);
        $this
            ->assertEquals(3, $store->increment('foo'));
    }
    ...
    public function testDecrementReturnsCorrectValues()
    {
        ...
        $table
            ...
            ->andReturn(null);
        $this
            ->assertFalse($store->decrement('foo'));
        ...
        $table
            ->shouldReceive('update')
            ->once()
            ->with(['value' => serialize(2)]);
        $this
            ->assertEquals(2, $store->decrement('bar'));
    }
